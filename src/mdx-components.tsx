...
export function useMDXComponents(components: MDXComponents): MDXComponents {
    return {
        ...
        h3: ({ children }) => (
            <h3
                id={`${children}`}
                className="mt-6 scroll-m-20 pb-2 text-md font-semibold tracking-tight first:mt-0 font-mono "
            >
        ...
        h4: ({ children }) => (
            <h4
                className="mb-2 mt-4"
                id={(children as string).toLocaleLowerCase()}
            >
        ...
        a: ({ children, href }) => (
            ...
            >
                {children}{" "}
                <GoArrowUpRight className="inline-block size-4 text-blue-500 group-hover:text-green-500" />
            </Link>
        ),
        ul: ({ children }) => (
